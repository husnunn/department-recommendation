...
@section('content')
    <x-admin.page-header
        ...
        description="Nama sekolah harus unik. Siswa memilih asal sekolah dari daftar ini."
        :breadcrumbs="[
    ...
    <div class="max-w-2xl rounded-2xl border border-outline-variant/30 bg-surface p-6 shadow-sm md:p-8">
        <form method="post" action="{{ route('admin.sekolah.store') }}" class="space-y-5">
            @csrf
            <div>
                <label class="mb-1 block text-sm font-medium text-on-surface-variant" for="nama_sekolah">Nama sekolah</label>
                <input id="nama_sekolah" name="nama_sekolah" type="text" value="{{ old('nama_sekolah') }}" required class="w-full rounded-lg border border-outline-variant px-3 py-2 text-on-surface focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
            ...
            <div>
                ...
                <input id="lokasi" name="lokasi" type="text" value="{{ old('lokasi') }}" placeholder="Kota / kabupaten" class="w-full rounded-lg border border-outline-variant px-3 py-2 text-on-surface focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
